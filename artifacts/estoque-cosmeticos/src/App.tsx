...
function OrderSummary({ order, featured = false }: { order: Order; featured?: boolean }) {
  ...
  return <section className={`rounded-2xl border p-6 ${featured ? 'border-emerald-200 bg-emerald-50/60' : 'border-border/80 bg-card shadow-[0_4px_22px_rgba(91,44,61,.035)]'}`}>
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
      <div>
        <p className={`font-mono text-[10px] uppercase tracking-[.18em] ${featured ? 'text-emerald-700' : 'text-muted-foreground'}`}>{featured ? 'movimentação finalizada' : isEntry ? 'entrada de estoque' : 'saída de estoque'}</p>
...
function OrdersPage() {
  ...
  const visibleOrders = orderFilter === 'all' ? orders : orders.filter((order) => order.type === orderFilter);
  ...
  const changeQuantity = (productId: number, amount: number) => {
    setSelected((current) => {
      ...
      if (orderType === 'out' && product && nextQuantity > product.stock) return current;
      if (nextQuantity <= 0) {
        const next = { ...current };
  ...
  return <div className="page-enter px-5 py-8 sm:px-8 lg:px-12 lg:py-11">
    <PageHeader eyebrow="estoque" title="Pedidos" description="Monte uma entrada ou saída com os produtos do catálogo e atualize o estoque quando finalizar." action={<Link href="/produtos" data-testid="link-orders-catalog" className="inline-flex w-fit items-center gap-2 rounded-xl border border-border bg-card px-4 py-3 text-xs font-extrabold text-foreground transition hover:bg-muted"><LayoutGrid size={16} /> Ver catálogo</Link>} />
    <div className="grid gap-5 xl:grid-cols-[1.18fr_.82fr]">
      <section className="rounded-2xl border border-border/80 bg-card p-5 shadow-[0_4px_22px_rgba(91,44,61,.035)] sm:p-6">
        <div className="mb-5 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-muted-foreground">catálogo integrado</p><h2 className="mt-1 text-lg font-extrabold tracking-[-.03em]">{orderType === 'in' ? 'Montar entrada' : 'Montar saída'}</h2></div><span className="text-xs text-muted-foreground">{products.length} {products.length === 1 ? 'produto disponível' : 'produtos disponíveis'}</span></div>
        <div className="mb-5 grid grid-cols-2 gap-2 rounded-xl bg-muted p-1"><button type="button" onClick={() => changeOrderType('in')} data-testid="button-order-type-in" className={`inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold transition ${orderType === 'in' ? 'bg-card text-emerald-700 shadow-sm' : 'text-muted-foreground'}`}><ArrowDownToLine size={15} /> Entrada</button><button type="button" onClick={() => changeOrderType('out')} data-testid="button-order-type-out" className={`inline-flex items-center justify-center gap-2 rounded-lg py-2.5 text-xs font-bold transition ${orderType === 'out' ? 'bg-card text-orange-700 shadow-sm' : 'text-muted-foreground'}`}><ArrowUpFromLine size={15} /> Saída</button></div>
